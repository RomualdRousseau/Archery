...
public class Dictionary
{
    ...
    {
        public WeightedString(String v) {
            this.value = v;
            this.weight = 1.0;
        }
    ...
    public void loadFromFile(String fileName) throws IOException, ParseException {
        ...
        for(int i = 0; i < jsonDefinitions.size(); i++) {
            ...
            for(int j = 0; j < jsonPatterns.size(); j++) {
                ...
                definition.patterns().add(new WeightedString((String) jsonPattern.get("value"), (Double) jsonPattern.get("weight")));                
            }
            ...
            for(int j = 0; j < jsonWords.size(); j++) {
                ...
                definition.words().add(new WeightedString((String) jsonWord.get("value"), (Double) jsonWord.get("weight")));                
            }
    ...
    private DataSet buildDataSet() {
        ...
        for(Definition definition: this.definitions) {
            ...
            //     if(firstPattern) {
            //         patternString = pattern.getValue();
            //         firstPattern = false;
            //     }
            //     else {
            //         patternString += "|" + pattern.getValue();
            ...
            for(WeightedString pattern: definition.patterns()) {
                for(WeightedString word: definition.words()) {
                    dataset.addRow(new DataRow()
                        ...
                        .addFeature(new FuzzyFeature(word.getValue()))
        ...
        for(DataRow row: dataset.rows()) {
            ...
            if(regexFeature.getValue().equals(".*")) {
                regexFeature.setValue(buildNegPattern);
            }
        }
